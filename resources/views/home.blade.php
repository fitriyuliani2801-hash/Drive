    ...
    @if($featuredArticles->isNotEmpty())
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-7">
                <a href="{{ route('articles.show', $primaryHeadline->slug) }}" class="group block relative bg-slate-950 rounded-3xl overflow-hidden shadow-xl border border-slate-200 hover:shadow-2xl transition-all duration-500">
                    <div class="relative h-[440px] w-full overflow-hidden bg-slate-900">
                        ...
                        <div class="absolute top-5 left-5 flex flex-wrap items-center gap-2 z-10">
                            <span class="px-3.5 py-1 rounded-full font-black text-white text-xs shadow-md uppercase tracking-wider font-heading bg-crimson-700">
                                {{ $primaryHeadline->category->name }}
                            </span>
                            <span class="px-3.5 py-1 rounded-full font-black text-amber-300 text-xs bg-slate-950/80 backdrop-blur-md border border-amber-400/40 flex items-center gap-1.5 font-heading">
            ...
            <div class="lg:col-span-5 space-y-3">
                @foreach($sideHeadlines as $index => $secHeadline)
                    <a href="{{ route('articles.show', $secHeadline->slug) }}" class="bg-white p-4 rounded-2xl border border-slate-200/90 hover:border-crimson-600 transition-all group flex items-start gap-3.5 shadow-xs hover:shadow-md block">
                        <!-- Number Badge -->
                        <div class="w-8 h-8 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center font-black text-sm shrink-0 font-heading group-hover:bg-crimson-700 group-hover:text-white transition-colors">
                            #{{ $index + 2 }}
                        </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 space-y-8">
            ...
            <div class="space-y-6">
                @foreach($latestArticles as $art)
                    <div class="bg-white rounded-3xl p-5 border border-slate-200/90 hover:border-brand-500 transition-all flex flex-col sm:flex-row gap-5 shadow-xs hover:shadow-md group">
                        
                        <!-- Thumbnail Image -->
                        @if($art->image_path)
                            <div class="w-full sm:w-48 h-40 sm:h-36 rounded-2xl overflow-hidden bg-slate-100 shrink-0 border border-slate-200 relative">
                                ...
                                <div class="absolute top-2 left-2">
                                    <span class="px-2.5 py-0.5 rounded-full font-black text-white text-[10px] uppercase font-heading bg-crimson-700 shadow-xs">
                                        {{ $art->category->name }}
                                    </span>
                                </div>
                            </div>
                        @else
                            <div class="w-full sm:w-48 h-40 sm:h-36 rounded-2xl bg-blue-50 flex items-center justify-center text-brand-400 shrink-0 relative border border-blue-100">
                                <i class="ri-newspaper-line text-4xl"></i>
                                <div class="absolute top-2 left-2">
                                    <span class="px-2.5 py-0.5 rounded-full font-black text-white text-[10px] uppercase font-heading bg-crimson-700">
                                        {{ $art->category->name }}
                                    </span>
                                </div>
                            </div>
        ...
        <div class="lg:col-span-5 space-y-8">
            ...
            <div class="bg-gradient-to-br from-brand-950 to-navy-900 text-white rounded-3xl p-6 border border-brand-900 shadow-md space-y-4">
                ...
                <div class="grid grid-cols-3 gap-2 text-center text-xs">
                    <div class="p-2 rounded-xl bg-blue-900/60 border border-blue-700">
                    ...
                    <div class="p-2 rounded-xl bg-blue-950 border border-blue-900">
            ...
            <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-xs space-y-4">
                ...
                <div class="space-y-2">
                    @foreach($categories as $cat)
                        <a href="{{ route('articles.index', ['category' => $cat->slug]) }}" class="flex items-center justify-between p-3 rounded-2xl bg-slate-50 hover:bg-red-50 hover:text-crimson-700 transition-all font-heading text-xs font-bold border border-slate-100 group">
                            <span class="flex items-center gap-2 text-slate-800 group-hover:text-crimson-700">
                                <i class="{{ $cat->icon }} text-brand-600"></i> {{ $cat->name }}
                            </span>
                            <span class="px-2 py-0.5 rounded-full bg-slate-200 group-hover:bg-crimson-700 group-hover:text-white text-slate-700 text-[10px]">
                                {{ $cat->articles_count }}
                            </span>
                        </a>
                    @endforeach
